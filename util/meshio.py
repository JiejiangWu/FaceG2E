...
def write_obj(obj_name, vertices, triangles=None, colors=None, normals=None,
  ...
  if check(triangles):
    triangles = triangles + 1 - np.min(triangles)
  if check(normal_idxs):
    normal_idxs = normal_idxs + 1 - np.min(normal_idxs)
  if check(texcoord_idxs):
    texcoord_idxs = texcoord_idxs + 1 - np.min(texcoord_idxs)
  ...
  with open(obj_name, 'w') as f:
    if mtllib:
      ...
      with open(lux_path + '.mtl', 'w') as mf:
        ...
        if uv_name is None:
          uv_name = os.path.split(lux_path)[-1] + '.png'
        mf.write('map_Kd {}\n'.format(uv_name))
