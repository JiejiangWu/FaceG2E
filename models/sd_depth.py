...
class StableDiffusion(nn.Module):
    def __init__(self, device, model_name='stabilityai/stable-diffusion-2-depth', concept_name=None, concept_path=None,
    ...
    def train_step(self, text_embeddings, inputs, depth_mask, mask, guidance_scale=100):
        ...
        if not self.latent_mode:
            ...
            latents = self.encode_imgs(pred_rgb_512 * mask)
            depth_mask = F.interpolate(depth_mask, size=(64, 64), mode='bicubic',
                                       align_corners=False)
        else:
            latents = inputs
        ...
        # grad = utils.gaussian_blur_latent(grad, 5, 2)
    ...
    def prompt_to_img(self, prompts, depth_mask, height=512, width=512, num_inference_steps=50, guidance_scale=7.5,
        ...
        depth_mask = F.interpolate(depth_mask, size=(height // 8, width // 8), mode='bicubic',
                                   align_corners=False)
